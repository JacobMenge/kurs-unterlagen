...
1. Netzwerk anlegen (mit einem sinnvollen Namen, passend zum Projektordner).
...
??? info "Projektname fixieren"
    ...
    === "Windows PowerShell"
        ```powershell
        $env:COMPOSE_PROJECT_NAME = "meinprojekt"
        docker compose up -d
        ```

    === "Windows CMD"
        ```cmd
        set COMPOSE_PROJECT_NAME=meinprojekt
        docker compose up -d
        ```
...
## Praktischer Arbeitsfluss mit Compose
